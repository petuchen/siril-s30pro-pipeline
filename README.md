...
[![Version](https://img.shields.io/badge/version-2.7.5-blue)](CHANGELOG.md)
...
| 2.5.2 | Fixed Batch stacking aborting on a transient logging timeout mid-run — logging calls inside the batch loop now fail safe instead of killing however many batches were already stacked. |
...
| 2.2.1 | Fixed a process-crashing abort (SIGABRT) that could hit when navigating to another stage right after one finished — a background preview-fetch thread could be destroyed while Qt still considered it running. Added `wait()` before every worker reference drop/reassignment. |
| 2.2.0 | Added: Denoise auto-detects and prefers an in-place-converted CoreML `.mlpackage` on Mac, running inference through coremltools directly instead of ONNX Runtime — faster and sidesteps the 3.x compile failure. |
...
| 2.0.4 | Improved: a not-yet-run stage's split preview now shows the same image on both sides instead of leaving the right half blank/black. |
| 2.0.3 | Fixed: "Run this stage" could stall with nothing happening, even on the first stage — a regression from 2.0.2 (the startup preview fetch and a stage-execution run could hit Siril's connection from two threads at once). Runs now queue behind an in-flight preview fetch instead of racing it. |
...
| 1.49.0 | New 🎨 button on each row of "Select objects to show..." opens a per-object style editor — marker style, colors/thickness, cross geometry, and label lines, all independent of the Annotation style panel and applying immediately to the preview. "Reset to panel default" discards an override; Cancelling the dialog undoes style edits too, not just visibility changes. Overrides are a preview/export-time adjustment — re-running Annotate regenerates every object from the panel defaults. |
| 1.48.0 | Annotate panel reorganized into a clear 3-step flow — "① Objects to show" (catalogue/constellation settings), "② Annotation style" (marker style, Circle/Open Cross panels, Label detail), then run/update/save actions. Open Cross style now sits after the object catalogue section, and gets a new "Label distance (× radius)" control for extra breathing room between the label and the marker center, separate from arm length. |
| 1.47.0 | Annotate: object labels can now show more than the name — a "Label detail" panel adds optional OpenNGC-sourced lines (object type, magnitude, constellation, apparent size) plus a custom-lines list for your own notes, each on its own line below the name. Lines align to whichever side of the marker the label sits on, and for Open Cross style the N/S arm stretches to reach a taller multi-line label. |
